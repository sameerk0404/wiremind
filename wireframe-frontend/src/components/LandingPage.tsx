...
import { convertImageToWireframe } from '../api/wireframeApi';
...
const LandingPage: React.FC<LandingPageProps> = ({ onGenerateWireframe }) => {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(true);
    try {
      const result = await convertImageToWireframe(file);
      if (result.errors?.length) {
        alert(result.errors[0]);
        return;
      }
      onGenerateWireframe(`Converting image: ${file.name}`);
    } catch (error) {
...
Or upload an image to convert it into a wireframe!"
                    value={description}
